{
  ...
  processColumns: function() {
    ...
    Ext.each(this.columns, function(c, i){
      ...
      this.normalizeSummaryRenderer(c);
  ...
  buildStore: function() {
    var store = Ext.create('Ext.data.Store', Ext.apply({
      ...
      remoteGroup: true,
      remoteSort: true,
      buffered: this.enablePagination,
  ...
  normalizeSummaryRenderer: function(c) {
    if (!c.summaryRenderer) return;

    var name, args = [];

    if ('string' === typeof c.summaryRenderer) {
      name = c.summaryRenderer.camelize(true);
    } else {
      name = c.summaryRenderer[0];
      args = c.summaryRenderer.slice(1);
    }

    // First check whether Ext.util.Format has it
    if (Ext.isFunction(Ext.util.Format[name])) {
       c.summaryRenderer = Ext.Function.bind(Ext.util.Format[name], this, args, 1);
    } else if (Ext.isFunction(this[name])) {
      // ... then if our own class has it
      c.summaryRenderer = Ext.Function.bind(this[name], this, args, 1);
    } else {
      // ... and, as last resort, evaluate it (allows passing inline javascript function as renderer)
      eval("c.summaryRenderer = " + c.summaryRenderer + ";");
    }
  },
